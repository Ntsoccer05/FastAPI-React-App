...
export const Card = (Props: Props) => {
  ...
  useEffect(() => {
    ...
    switch (selectedValue) {
      case "古い順":
        sortedCards &&
          sortedCards.sort((beforeVal, nextVal) => {
      ...
      case "更新順":
        sortedCards &&
          sortedCards.sort((beforeVal, nextVal) => {
            if (
              new Date(beforeVal.updated_at as string) >
              new Date(nextVal.updated_at as string)
            ) {
              return -1;
            } else {
              return 1;
      ...
      <div className="w-full bg-gradient-to-bl from-blue-50 to-violet-50 ">
        <div className="container mx-auto p-4 h-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 xl:grid-cols-4 gap-4">
          {cards?.map((post) => {
            return (
              post.can_post && (
                <div className="bg-white rounded-lg border p-4" key={post.id}>
                  <img
                    ...
                    <div className="grid grid-cols-2">
                      ...
                      >
                        <svg
                          ...
                          stroke="currentColor"
                          className={
                            "w-6 h-6 justify-end fill-red-500" +
                            (like && "fill-red-500")
                          }
                          onClick={liked}
                        >
                          <path
                            className={like ? "fill-pink-500" : ""}
                            strokeLinecap="round"
